...
public class MethodReference {
			/*
			 * 람다식의 메서드 참조(Method Reference) 문법
			 * 	- 메서드 참조는 람다식이 수행하는 일이 "어떤 메서드를 그대로 호출하는 것"일때
			 *	  그 메서드의 이름만으로 람다식을 배치하는 문법
			 *	- 기존 람다식 구문을 더욱 간결하게 표현하기 위하여 사용한다.
			 *	  ex) (str) -> {return str.length();}
			 *		  String ::length;
			 *
			 *	- 람다식을 통해 구현한 기능이 이미 정의되어 있는 다른 메서드의 기능을 호출하고,
			 *	  반환시키거나 출력하거나 단순한 역할을 수행할 때,
			 *	  그 다른메서드의 이름만 추가시키는 형태로 작성하는 구문.
			 *
			 * 메서드 참조의 종류
			 * 1. static 메서드 참조
			 * 2. 일반 메서드 참조
			...
			public static void main(String[] args) {
				...
				System.out.println(strLength.apply("이건 일반메서드 참조"));
				...
				equalsToTitle = str -> title.equals(str); // title : 익명 내부클래스에서 지역변수 사용중이기 때문에 상수 취급
				equalsToTitle = title::equals;
				
				System.out.println(equalsToTitle.test("소날기"));
