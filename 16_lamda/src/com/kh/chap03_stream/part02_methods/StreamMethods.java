...
public class StreamMethods {
			public static void main(String[] args) {
				 ...
				 * 	-> 산술 연산 관련된 최종 처리 메서드
				 * 4) Matching 계열 : anyMatch, allMatch, nonMatch...
				...
				// 중간처리 메서드 (distinct, filter, forEach(중간 아님))
				List<Integer> list = Arrays.asList(5,1,4,2,1,6,4,5,2,3);
				...
				list2.stream().map((i)->i+"!").forEach(System.out::print);  // stream <String>
				...
				// 4-2) Collectiors.toSet() : 스트림의 데이터를 Set으로 변환
				...
				// 5) Calculating 계열
				//	- 산술 연산시 사용하는 메서드들로 기본자료형 스트림에 존재
				// 5-1) sum()
				...
				// 5-3) summaryStatistics : 스트림 요소의 통게값을 가진 객체를 반환해주는 메서드
				// 		통계값 - > 총 개수, 합, 평준, 최소, 최대
				IntSummaryStatistics summary=
						IntStream.of(15,65,48,72,32,15,98,78).summaryStatistics();
				...
				int result4 = Arrays.asList(1,6,5,4,7,9,51,3,456,12,748,5,123)
									.stream()
									.reduce(0, (max,n)->{
				...
				List<Member> arr = Arrays.asList(new Member("홍길동", 35),
						
						new Member("신사임당", 40), new Member("세종", 45),
				...
				Member maxAgePerson =
						arr.stream().reduce((m1,m2)->{ // 초기값 생략시 스트림의 첫번째 요소가 초기값으로 설정
				...
				bool = Arrays
						.asList("홍길동11","123","가나다")
						.stream()
						.noneMatch(s->s.length()>4);
				System.out.println(bool);
				
				// 7-3) allMatch : 스트림의 모든 요소가 true인 경우 true
				bool = Arrays
						.asList("홍길동","123","가나다")
						.stream()
				...
				str = Arrays.asList("홍길동" , "123", "159357").stream().filter(s->s.length()<=4).findAny().get();
