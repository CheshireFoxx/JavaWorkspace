...
public class C_StringTokenizer {
		public void method() {
			String str = "Java,Oracle,JDBC,HTMI,String";
			
			// 구분자를 제시해서 문자열을 구분자 기준으로 분리 시키는 방법
			// 방법 1. 문자열.split(구분자) : String[]    => 메모리 많이 먹음
			// 		- 분리된 문자열들을 String[]에 담아서 관리해주는 메서드
			String[] arr = str.split(",");
			
			System.out.println(Arrays.toString(arr));
			
			// 방법 2. 분리된 각각의 문자열들을 토큰으로 취급하여 관리하는 방법     => 메모리 절약 가능 / 중간 문자열만 출력 불가
...
//			}
			...
			while(stn.hasMoreTokens()) { // stn에 토큰이 1개 이상 있는경우 true/ false
				System.out.println(stn.nextToken());
			}
