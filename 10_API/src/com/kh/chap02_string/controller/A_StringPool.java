...
public class A_StringPool {
			...
			// - 문자열 데이터가 담기는 영역으로, Heap 메모리 영역에 항상 고정적으로 할당되어 있다.
			public void method() {
				String str1 = new String ("hello");
				String str2 = new String ("hello");
				
				System.out.println(str1==str2); // false(주소값 비교)
				System.out.println(str1); // toString 오버라이딩
				System.out.println(str2);
				
				System.out.println(str1.equals(str2)); // true(오버라이딩)
			...
			public void method2() {
				 ...
				 * 
				 * 문자열 상수풀 동작 방식(JVM)
				 * 1. 프로그램 실행 도중 문자열 리터럴을 만나면, 문자열 리터럴값이 이미
				 *	  상수풀에 보관되어 있나 검사를 진행(intern)
				 * 2. 만약 상수풀에 저장된 리터럴값이 아니라면, 문자열의 hashCode 함수를 호출하고
				 * 	  그 값을 상수풀의 주소값으로 활용하여 값을 저장한다.
				 * 3. 만약 이미 상수풀에 그 값이 저장되어 있다면, 저장되어 있는 주소값을 반환한다. (얕은 복사) 
				 */
				
				
				/*
				 * hashCode 메서드를 오버라이딩한 이유
				 *  - 문자열은 문자열 기반 hashCode값을 상수풀상의 주소값으로 사용하고 있다.
				 *    따라서 상수풀 안에 데이터를 저장할 때 문자열을 해시코드값으로 변환한 후 그 자리에
			 ...
			 * 문자열 보다는 문자열 기반 가변 클래스인 (StringBuffer , StringBuilder)를
			 * 사용하는것을 권장 
			 */
