...
public class AirPolution {
			public static void main(String[] args) {
				 ...
				 *  - API 서버
				 *  	- URL로 데이터에 대한 요청을 보내면 응답데이터를 반환해주는 서버로, 어플리케이션들 중 데이터나 서비스를 제공하는 역할을 API서버라고 부른다.(카카오, 네이버 등)
				 * 		- A어플리케이션이 B어플리케이션에 필요한 데이터가 있는 경우, B어플리케이션의 API서버를 통해 요청(request)를 보내고, B API 서버는 요청에 대한 응답(response)을 돌려준다.
				 * 		- 단, API서버 요청시에는 해당 API서버가 정한 규칙(interface)에 맞춰 요청을 보내야 한다.
				 * 
				 * 2. API 서버 요청 방식(Http Mthod)
				 * 	- API 서버에서 지정한 규칙에 따라야 하며, 전통적인 방식의 API요청방법은 2가지가 있다.
				 * 
				 * 	1) Get 방식
				 *		- URL에 요청변수(querParameter)를 붙여서 요청하는 방식
				 *		- 요청 변수는 key = value 형태로 작성하며 여러개의 요청변수가 있을 경우 &로 구분힌다.
				 ...
				 * 		- ex) Http://khedu.co.kr/data (쿼리스트링은 url에 노출되지 않는다)
				 * 
				 * 3. API서버 응답 데이터 형식 및 처리 방법
				 * 	1) XML유형의 데이터
				 * 		<course>
				 * 		//...
				 * </course>
