...
public class NetworkTest {
		...
		public void test2() {
			...
			/*
			 * URL
			 *  - 인터넷상에 전재하는 서버들의 자원에 접근할 수 있는 주소.
			 *  - 자바에서는 URL 클래스를 통해 URL 자원을 다룰 수 있다.
			 *  - 실제 URL 자원에 대한 객체를 생성하여 자원에 연결하고 데이터를 읽어올 수 있다.
			 * 
			 * URL구조
			 *  - https://kh-academy.co.kr/project/teamView.kh?no=5406&cpage=1
			 * 
			 * protocal : 통신규약(http, https, smtp, file, ...)
			 * hostname : 도메인 이름(ip 주소) kh-academy.co.kr
			 * port : 서비스 번호.호스트 내에 특정 서비스를 가리키는 논리적인 번호.
			 * 		  포트번호 별로 다양한 서비스를 제공할 수 있다.
			 * 		  각 프로토콜의 기본포트는 생략이 가능하낟.
			 * 		  https의 기본퐅, : 443, http의 기본 포트 : 30, ftp의 기본포트 : 20 ...
			 * path : 자원경로. hostname과 port 내부에 자원이 존재하는 세부 경로
			 * 		  /project/teamView.kh
			...
			try {
				URL url = new URL("https://kh-academy.co.kr/project/teamView.kh?no=5406&cpage=1");
